...
const ChannelList = observer(() => {
  ...
  const onDisconnect = () => {
    window.electron.ipcRenderer.sendMessage('disconnectFromChannel', [
      selectedServer,
      targetChannel,
    ]);
  };
  ...
  const onConnect = () => {
    window.electron.ipcRenderer.sendMessage('connectToChannel', [
      selectedServer,
      targetChannel,
    ]);
  };
  ...
  return (
    <Box className="ChannelList">
      ...
      <Typography className="FlexColumnHeading">{selectedServer}</Typography>
